...
class UserRegisterRequest extends FormRequest
{
    ...
    {
        return [
            'first_name' => 'required|regex:/^[a-zA-Z]+(?:[\s.]+[a-zA-Z]+)*$/|min:2|max:150',
            'last_name' => 'required|regex:/^[a-zA-Z]+(?:[\s.]+[a-zA-Z]+)*$/|min:2|max:150',
            'username' => ['required', 'alpha_dash', Rule::unique('users', 'username'), 'min:2', 'max:150'],
            'email' => ['required', 'email:rfc,dns', Rule::unique('users', 'email'), 'min:5', 'max:150'],
            'password' => ['required', Password::min(8)->letters()->mixedCase()->numbers()->symbols(), 'max:150'],
            'confirm_password' => ['required', 'same:password'],
            'picture' => ['required', 'mimes:jpg,jpeg,png', File::image()->min(1024)->max(10240)]
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'first_name.max' => 'Please first name max length is 150 digit',
            ...
            'last_name.max' => 'Please last name max length is 150 digit',
            ...
            'picture.file' => 'Please input only image file type min file size is 1 MB and max file size is 10 MB'
        ];
    }
